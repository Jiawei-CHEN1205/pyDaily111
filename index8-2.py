###### 飞机大战小游戏 抽象基类后的代码（简洁版1.0）
import pygame
import random
from pygame.locals import * # *号表示的是导入全部

'''001、抽象一个基类 表示两个飞机'''
class BasePlane(object): # 基类的话 两个飞机的图片就需要作为参数传入而不是定义好的了 所以会多一个参数
    def __init__(self,screen,imagePath) -> None:
        self.screen = screen
        self.image = pygame.image.load(imagePath)
        self.image = pygame.transform.scale(self.image,(50,50))# 改变图片的大小
        self.listBullet = [] # 存放子弹的列表(两者都有)
        pass
    ...
    pass   
'''002、类 继承 玩家和敌机'''
class HeroPlane(BasePlane): #玩家
    def __init__(self,screen) -> None: 
        imagePath = 'C:/A--HITsz_files/4--learnPY/qiuzhi_feiji/hero1.jpg'
        super(HeroPlane,self).__init__(screen,imagePath)
    # 飞机的默认位置(hero)
        self.x = 100
        self.y = 450
        pass
...
class EnemyPlane(BasePlane): #敌机
    def __init__(self,screen) -> None:   
        imagePath = 'C:/A--HITsz_files/4--learnPY/qiuzhi_feiji/enemy1.jpg'
        super(EnemyPlane,self).__init__(screen,imagePath)
    # 敌人飞机的默认位置
        self.x = 100
        self.y = 100
        pass
    def Shoot(self): # 重写shoot
        num = random.randint(1,100)
        if num == 13:# 敌机的发射子弹 随机发射子弹
            # 如果随机数是13，就创建一个新的敌机的子弹对象（实例化）
            newBullet = BaseBullet(self.x,self.y,self.screen,'enemy') # 更改类型enemy
            # 追加子弹对象append列表，因此列表中的元素都是一个个的子弹对象，每次调用一次shoot 都会增加一个子弹实体：
            self.listBullet.append(newBullet)
        pass
    def Display(self):
...
class BaseBullet(object):
    def __init__(self,x,y,screen,bulletType) -> None:
        self.Type = bulletType # 根据类型定义到底是hero还是enemy 从而子弹的性质也不一样
        '''
        x:飞机横坐标 y:飞机纵坐标 screen:主窗口
        '''
        self.screen = screen # 这里的screen之类的都是形式参数
        self.x = x+16  # 子弹的坐标self.x是相对于飞机的坐标位置而定义的,横坐标都是差不多的
        if self.Type == 'hero':
            self.y = y-15
    ...
    def MoveBullet(self):
        if self.Type == 'hero':
            ...
            self.y += 3
        pass
    def Judge(self): # 子弹越界判断
        if self.y < 0  or self.y > (500-21):
            return True
        else:
            return False
...
'''005、主函数部分 包括了定义背景图片、背景音乐 实例化飞机对象等等功能'''
def main():
    ...
    pygame.display.set_caption('飞机大战')# 设置一个title
    ...
    while True: # 一直循环 刷新页面
        # 设定要显示的内容
        screen1.blit(background,(0,0)) #背景图片
        # 显示玩家飞机的图片
        Hero1.Display() #调用 按照设定的x,y，显示玩家的位置
        # 获取键盘事件
        Key_control(Hero1) #传入实际的对象Hero1
        # 显示敌机飞机的图片
        enemy1.Display() #调用 按照设定的x,y，显示敌机的位置
        # 敌机发射子弹
        enemy1.Shoot()
        # 随机移动敌机:
        ...
        # 知道是循环的原因，又知道循环是为了检测键盘事件，可以让循环次数变少为100，而且变少后不影响程序对键盘的检测
        pass
    pass
...
#         x:飞机横坐标 y:飞机纵坐标 screen:主窗口
#         '''
#         self.x = x+16  # 子弹的坐标self.x是相对于飞机的坐标位置而定义的
#         self.y = y-30
#         self.screen = screen # 这里的screen之类的都是形式参数
#         # 导入子弹的图片18*30大小
...
#         self.y -= 1.5 # 坐标的变化 向上发射 数值大小影响到子弹的速度
#         pass
...
#     for event in evenList:
#         if event.type == QUIT:
#             # print('按下了退出 得分是{}'.format(scorePlay))
#             print('游戏退出')
#             exit()
#             pass
#         elif event.type == KEYDOWN:# KEYDOWN按下按键的关键字event.type
...
#         '''
#         self.x = x+16  # 子弹的坐标self.x是相对于敌机的坐标位置而定义的
#         self.y = y+30+50
#         self.screen = screen # 这里的screen之类的都是形式参数
#         # 导入子弹的图片18*30大小
